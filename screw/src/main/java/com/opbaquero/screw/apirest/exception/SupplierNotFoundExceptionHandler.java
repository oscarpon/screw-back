...
@ControllerAdvice
public class SupplierNotFoundExceptionHandler {

    @ExceptionHandler({SupplierNotFoundException.class})
    public ResponseEntity<Model404NotFoundErrorDTO> handlerApiResquestException(final SupplierNotFoundException ex){
        final ErrorObjectDTO error = new ErrorObjectDTO().code(HttpStatus.NOT_FOUND.toString()).description("Supplier not found").message(ex.getMessage());
