...
public class ProductNotFoundExceptionHandler {
    ...
    public ResponseEntity<Model404NotFoundErrorDTO> handlerApiResquestException(final ProductNotFoundException ex){
        final ErrorObjectDTO error = new ErrorObjectDTO().code(HttpStatus.NOT_FOUND.toString()).description("Product not found").message(ex.getMessage());
